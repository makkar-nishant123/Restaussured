...
class Log
{
	...
	private static void PageInfo(Config testConfig)
	{
		...
		String showNetExport = testConfig.getRunTimeProperty("NetExport");
		if (showNetExport != null && showNetExport.equalsIgnoreCase("true"))
		{
			Browser.getFireBugResponse(testConfig);
		}
		
		if (testConfig.recordPageHTMLOnFailure)
		{
			if (testConfig.driver != null && testConfig.testMethod != null)
			{
				File dest = Browser.getPageHTMLFile(testConfig);
				Browser.recordPageHTML(testConfig, dest);
			}
		}
	}
